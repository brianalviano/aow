...
<div class="min-h-screen bg-slate-950 flex flex-col">
    ...
    >
        ...
        <h1
            class="font-bold text-lg leading-tight text-slate-100 flex-1 text-center mr-8"
        >
            Request Menu
        </h1>
    </header>
    ...
    <main class="flex-1 w-full max-w-md mx-auto p-4 space-y-6">
        <!-- Form Section -->
        <section
            class="bg-slate-950 p-6 rounded-2xl border border-slate-800 shadow-sm"
        >
            <h2 class="font-bold text-slate-100 mb-1">Ingin menu apa?</h2>
        ...
        <section class="space-y-4">
            <h2 class="font-bold text-slate-100 px-1">Riwayat Request Anda</h2>

            {#if requests.length === 0}
                <div
                    class="bg-slate-950 p-8 rounded-2xl border border-dashed border-slate-700 flex flex-col items-center justify-center text-center"
                >
                    <div
                        class="size-12 bg-slate-800 rounded-full flex items-center justify-center text-slate-500 mb-3"
                    >
                        <i class="fa-solid fa-utensils text-xl"></i>
                    </div>
                    <p class="text-sm text-slate-400">
                        Belum ada request menu.
                    </p>
                </div>
            {:else}
                <div class="space-y-3">
                    {#each requests as req}
                        <div
                            class="bg-slate-950 p-4 rounded-xl border border-slate-800 shadow-sm flex flex-col gap-2"
                        >
                            <div class="flex items-start justify-between gap-3">
                                <h3
                                    class="font-bold text-slate-100 text-[15px] leading-tight flex-1"
                                >
                                    {req.name}
                                </h3>
                                <span
                            ...
                            {#if req.notes}
                                <p
                                    class="text-[13px] text-slate-400 line-clamp-2 italic"
                                >
                                    "{req.notes}"
                                </p>
                            {/if}
                            <div
                                class="flex items-center gap-1.5 text-[11px] text-slate-500 mt-1"
                            >
