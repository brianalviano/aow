...
class UpdateScheduleRuleRequest extends FormRequest
{
    ...
    {
        /** @var \App\Models\ScheduleRule|null $scheduleRule */
        $scheduleRule = $this->route('schedule_rule') ?? $this->route('scheduleRule');
        return [
            'user_id' => [
                'required',
                'uuid',
                Rule::unique('schedule_rules', 'user_id')
                    ->ignore($scheduleRule?->id, 'id')
            ],
