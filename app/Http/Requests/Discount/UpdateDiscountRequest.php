...
final class UpdateDiscountRequest extends FormRequest
{
    ...
    {
        $routeParam = $this->route('discount');
        $id = $routeParam instanceof Discount ? (string) $routeParam->getKey() : (string) $routeParam;

        return [
            'name' => ['required', 'string', 'max:255', Rule::unique('discounts', 'name')->ignore($id, 'id')],
