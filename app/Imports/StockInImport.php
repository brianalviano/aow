...
{
    private GoodsComeService $service;
    ...
    public function __construct(GoodsComeService $service, ?string $userId = null)
    {
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $row) {
            ...
            $unitCost = isset($row['unit_cost']) ? (int) $row['unit_cost'] : 0;
            $notes = isset($row['notes']) ? (string) $row['notes'] : null;
            $isVatRaw = isset($row['is_vat']) ? (string) $row['is_vat'] : '';
            $isVat = in_array(strtolower($isVatRaw), ['1', 'true', 'yes', 'y'], true);

            if ($warehouseId && $productId && $qty > 0) {
                $this->service->receiveGoods([
